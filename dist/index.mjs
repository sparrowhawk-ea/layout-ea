...
class Child {
    constructor(e, name, parent) {
        ...
        this.children = {};
        this.childMatches = {};
        this.path = (parent === null || parent === void 0 ? void 0 : parent.path) && !name.startsWith('#') ? parent.path + ' ' + name : name;
    ...
    parseIntOrWild(str, min, wild, sizes, error) {
        if (str)
            str = sizes[str] || str;
        return (str === null || str === void 0 ? void 0 : str.startsWith('*')) ? wild : str ? this.parseInt(str, min, error) : min;
    }
...
class ChildRef {
    constructor(ref, index, isWildcard = false, child) {
        ...
        this.isExplicit = (child === null || child === void 0 ? void 0 : child.e.id) && ref.endsWith('#' + child.e.id) || !ref;
        if (!ref)
            ref = String(index);
        else if (index > 0)
            ref += '/' + String(index);
        ...
        holders.forEach(holder => {
            ...
            if (!previousNode) {
                ...
                if (previousNode.childRef.isExplicit)
                    holder.childRef.die(parent, errLoc
...
class Impl {
    ...
    layoutDepthFirst(parent) {
        ...
        while (layout === null || layout === void 0 ? void 0 : layout.startsWith('@')) {
    ...
    makeName(e) {
        ...
        for (let i = 0; i < siblings.length; i++) {
            ...
            if (tag === sibling.tagName && sibling.dataset[this.dataPropIgnore] === undefined)
                tagCounter++;
        }
...
class Grid {
        ...
        const gridItemsByRow = layout.split(';').flatMap((rowStr, row) => {
            ...
            const itemTemplates = items.map((item, col) => {
                ...
                if (childRefTemplates.length === 0)
                    this.parent.die(errLoc + 'missing child reference', layout);
                return {
                    childRefTemplates,
            ...
            for (let cellsAboveRepeatedRow = this.cellsAboveRow[this.cellsAboveRow.length - 1]; rowRepeatContext.consumeAll || rowRepeatContext.iteration < rowRepeat; rowRepeatContext.iteration++) {
                const rowItems = itemTemplates.flatMap((itemTemplate, col) => this.parent.getChildRefs(itemTemplate.childRefTemplates, rowRepeatContext, this.errLoc(row, col))
    ...
    placeItems(row, rowItems, multiRowItems, gridWidth) {
        ...
        rowItems.forEach(item => {
            ...
            if (col + item.width > wall)
                item.childRef.die(this.parent, this.errLoc(row, col) + 'width ' + String(item.width)
    ...
    placeRows(rowItemsList) {
        ...
        multiRowItems.forEach(item => {
            if (item.height > 0)
                item.childRef.die(this.parent, this.errLoc(item.row, item.col) + 'height ' + String(item.height)
